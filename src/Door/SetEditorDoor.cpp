...
void C_SetEditorDoor::FirstTime()
{
    ...
    label = new C_Label(5, 10, 6, 1, "Tile W", "Tile Width", 0, 0, 0, 155, 155, 155, 155, 155, 155, 155, 155, 155);
    AddFormElement(label);
    label = new C_Label(5, 40, 6, 1, "Tile H", "Tile Height", 0, 0, 0, 155, 155, 155, 155, 155, 155, 155, 155, 155);
    AddFormElement(label);

    fTileWidth = new C_InputInteger(100, 10, 3, 1, ToString(levelEditor->GetGameSetup()->GetTileWidth()).c_str(), "Tile width");
    fTileHeight = new C_InputInteger(100, 40, 3, 1, ToString(levelEditor->GetGameSetup()->GetTileHeight()).c_str(), "Tile height");
    fTileWidth->SetMinMax(16, 256);
    fTileHeight->SetMinMax(16, 256);
    AddFormElement(fTileWidth);
    AddFormElement(fTileHeight);

    label = new C_Label(5, 70, 6, 1, "Map BG", "Map background color", 0, 0, 0, 155, 155, 155, 155, 155, 155, 155, 155, 155);
    AddFormElement(label);
    fMapColorRed = new C_InputInteger(100, 70, 3, 1, ToString(levelEditor->GetGameSetup()->GetMapBGCR()).c_str(), "Red 0 - 255");
    fMapColorRed->SetMinMax(0, 255);
    fMapColorGreen = new C_InputInteger(150, 70, 3, 1, ToString(levelEditor->GetGameSetup()->GetMapBGCG()).c_str(), "Green 0 - 255");
    fMapColorGreen->SetMinMax(0, 255);
    fMapColorBlue = new C_InputInteger(200, 70, 3, 1, ToString(levelEditor->GetGameSetup()->GetMapBGCB()).c_str(), "Blue 0 - 255");
    fMapColorBlue->SetMinMax(0, 255);
    AddFormElement(fMapColorBlue);
    AddFormElement(fMapColorGreen);
    AddFormElement(fMapColorRed);

    bOk = new C_Button(105, 100, 40, 20, "OK", alignCenter);
    title = "Set editor";
    width = 250;
    height = 130;
