...
const GLubyte C_TextureTGA::uTGAcompare[12] = {0,0,2, 0,0,0,0,0,0,0,0,0};
const GLubyte C_TextureTGA::cTGAcompare[12] = {0,0,10,0,0,0,0,0,0,0,0,0};

C_TextureTGA::C_TextureTGA() : C_TextureLoader("tga")
{
}

bool C_TextureTGA::Load(S_Texture * texture, const char * filename)				// Load a TGA file
{
	...
	{
		fprintf(stderr, "TGA file be type 2 or type 10 > %s\n", filename);
...
bool C_TextureTGA::LoadUncompressedTGA(S_Texture * texture, const char * filename, FILE * fTGA)	// Load an uncompressed TGA (note, much of this code is based on NeHe's
{																			// TGA Loading code nehe.gamedev.net)
	...
	{
		fprintf(stderr, "Could not read image data");		// Display Error
	...
	for(GLuint cswap = 0; cswap < (unsigned int)tga.imageSize; cswap += tga.bytesPerPixel)
	{
...
bool C_TextureTGA::LoadCompressedTGA(S_Texture * texture, const char * filename, FILE * fTGA)		// Load COMPRESSED TGAs
{
